...
//            );
        ...
        if(gamepad2.bWasPressed()){
            shooterTimer.reset();
            intakeTimer.reset();
            if(shooterTimer.seconds()<shooterWaitTime){
                shooterSubsystem.setPowerTo(1);
                if(intakeTimer.seconds()<intakeWaitTime){intakeMotor.setPower(-0.65);}
            }
        }
